...
if (
  ...
  server = https.createServer({ key, cert }, app).listen(port, () => {
    logger.info(`🔒 HTTPS server is running at https://${process.env.HOSTNAME}:${port} [${process.env.NODE_ENV}]`);
  ...
  server = app.listen(port, () => {
    ...
    if (!sslKeyPath || !fs.existsSync(sslKeyPath)) {
      logger.warn(
        "No SSL key found (set SSL\_KEY in .env); running in HTTP mode."
      );
    }
    if (!sslCertPath || !fs.existsSync(sslCertPath)) {
      logger.warn(
        "No SSL cert found (set SSL\_CERT in .env); running in HTTP mode."
      );
    }
  });
}
