...
class BloomFilterService extends EventEmitter {
    ...
    constructor(config: BloomFilterConfig) {
        ...
        this.saveToDiskDebounced = debounce(() => {
            this.saveToDisk().catch((err) =>
                logger.error("Failed to persist Bloom filter:", err)
            );
        }, 10_000);
    }
    ...
    public async initialize(): Promise<void> {
        ...
        try {
            ...
            logger.info(`🌸 Bloom filter stats: ${bloom.length} bits, expected FPR: ${this.config.falsePositiveRate}`);
        } catch (error) {
    ...
    public startPeriodicRefresh(): void {
        ...
        logger.info(`🌸 Bloom filter periodic refresh started (every ${this.config.refreshIntervalMs}ms)`);
    }
